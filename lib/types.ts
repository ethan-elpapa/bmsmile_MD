...
export type WeekItem = {
  id: string;
  /**
   * **적은 사람의 Member.id.** 컨펌 요청도 적은 사람이 있다 — 그게 있어야
   * "자기가 적은 것만 고칠 수 있다" 는 규칙을 서버가 검사할 수 있다.
   * 빈 문자열은 사람별 칸이 생기기 전의 옛 항목뿐이고, 그건 팀장만 고친다.
   */
  by: string;
  /** true 면 **컨펌 요청** 줄에 뜬다 — 팀장이 결정해 줘야 하는 것. */
  ask: boolean;
  text: string;
  /** 팀장이 완료로 표시했는지. 미완료가 기본이고, **컨펌 요청에만 뜬다**. */
  done: boolean;
  /** 완료로 바꾼 날 YYYY-MM-DD */
  doneAt: string;
};
...
/** 옛 항목의 작성자 자리. 사람별 칸이 생기기 전에 적힌 것들이 이 값을 갖는다. */
